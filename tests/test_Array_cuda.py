...
AV = TypeVar(
    "AV",
    _containers_cuda.ArrayView_int,
    _containers_cuda.ArrayView_float,
    _containers_cuda.ArrayView_complex,
    _containers_cuda.VectorView_int,
    _containers_cuda.VectorView_float,
    _containers_cuda.VectorView_complex,
)

# Type variable constraining the const array view types
CAV = TypeVar(
    "CAV",
    _containers_cuda.ArrayView_int_const,
    _containers_cuda.ArrayView_float_const,
    _containers_cuda.ArrayView_complex_const,
    _containers_cuda.VectorView_int_const,
    _containers_cuda.VectorView_float_const,
    _containers_cuda.VectorView_complex_const,
)
...
def test_dlpack(array_type: type[A], data: st.DataObject) -> None:
    ...
    assert view_cupy.shape == dims, f"Expected shape {dims}, got {view_cupy.shape}"
    assert view_cupy.dtype == array_cupy.dtype
    assert all(view_cupy[i] == array[i] for i in range(len(array))), "Data mismatch in CuPy array"
