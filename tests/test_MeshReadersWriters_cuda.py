...
def test_resolveMeshType_distributed(file_path: str, expected_vertices: int, expected_cells: int, tmp_path: Path) -> None:
    ...
    mesh_class = pytnl.meshes.DistributedMesh[pytnl.meshes.Mesh[topology, Cuda]]  # type: ignore[type-arg, valid-type]
    ...
    with pytest.raises(RuntimeError):
        pytnl.meshes.resolveMeshType(f"invalid{suffix}")
    reader, mesh = pytnl.meshes.resolveMeshType(str(output_pvtu), device_type=Cuda)
    assert isinstance(reader, reader_class), reader
    assert isinstance(mesh, mesh_class), mesh
    # Check mesh entities
    ...
    with pytest.raises(RuntimeError):
        pytnl.meshes.resolveAndLoadMesh(f"invalid{suffix}")
    reader, mesh = pytnl.meshes.resolveAndLoadMesh(str(output_pvtu), device_type=Cuda)
    assert isinstance(reader, reader_class), reader
    assert isinstance(mesh, mesh_class), mesh
    # Check mesh entities
    local_mesh = mesh.getLocalMesh()  # pyright: ignore
