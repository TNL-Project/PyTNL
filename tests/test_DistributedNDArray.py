...
type DistributedNDArrayView = (
    pytnl._containers.DistributedNDArrayView_1_complex
    | pytnl._containers.DistributedNDArrayView_2_complex
    | pytnl._containers.DistributedNDArrayView_3_complex
    | pytnl._containers.DistributedNDArrayView_1_int
    | pytnl._containers.DistributedNDArrayView_2_int
    | pytnl._containers.DistributedNDArrayView_3_int
    | pytnl._containers.DistributedNDArrayView_1_float
    | pytnl._containers.DistributedNDArrayView_2_float
    | pytnl._containers.DistributedNDArrayView_3_float
)
type DistributedNDArrayView_const = (
    pytnl._containers.DistributedNDArrayView_1_complex
    | pytnl._containers.DistributedNDArrayView_2_complex_const
    | pytnl._containers.DistributedNDArrayView_3_complex_const
    | pytnl._containers.DistributedNDArrayView_1_int_const
    | pytnl._containers.DistributedNDArrayView_2_int_const
    | pytnl._containers.DistributedNDArrayView_3_int_const
    | pytnl._containers.DistributedNDArrayView_1_float_const
    | pytnl._containers.DistributedNDArrayView_2_float_const
    | pytnl._containers.DistributedNDArrayView_3_float_const
)
...
def test_pythonization() -> None:
    assert DistributedNDArray[1, int] is pytnl._containers.DistributedNDArray_1_int
    assert DistributedNDArray[2, float] is pytnl._containers.DistributedNDArray_2_float
    assert DistributedNDArray[3, complex] is pytnl._containers.DistributedNDArray_3_complex

    assert DistributedNDArray[1, int, devices.Host] is pytnl._containers.DistributedNDArray_1_int
    assert DistributedNDArray[2, float, devices.Host] is pytnl._containers.DistributedNDArray_2_float
    assert DistributedNDArray[3, complex, devices.Host] is pytnl._containers.DistributedNDArray_3_complex
...
def test_typedefs(dim: DIMS, value_type: type[VT]) -> None:
    ...
    """
    ndarray_class = DistributedNDArray[dim, value_type]  # type: ignore[type-arg,valid-type]
...
def test_setSizes(shape: tuple[int, ...]) -> None:
    ...
    a = DistributedNDArray[dim, int]()  # type: ignore[index]
    a.setSizes(*shape)
...
def test_setLike(shape: tuple[int, ...]) -> None:
    ...
    b = DistributedNDArray[dim, int]()  # type: ignore[index]
...
def test_str_repr(value_type: type[VT], shape: tuple[int, ...]) -> None:
    ...
    array_type = DistributedNDArray[dim, value_type]()  # type: ignore[index]
    array_type.setSizes(*shape)

    # Check that `__str__` contains the correct value type and shape
    str_output = str(array_type)
    expected_str = f"DistributedNDArray[{dim}, {value_type.__name__}, Host]({', '.join(str(x) for x in shape)})"
...
def test_dlpack(shape: tuple[int, ...]) -> None:
    ...
    array = DistributedNDArray[dim, int]()  # type: ignore[index]
    ...
    assert array_np.shape == local_shape, f"Expected shape {local_shape}, got {array_np.shape}"
    ...
    ndidx = np.ndindex(local_shape)

    # Modify NumPy array and verify NDArray reflects the change
    idx = next(ndidx)
    array_np[idx] = 99
    assert array.getLocalView()[idx] == 99, "NumPy array modification not reflected in NDArray"
    ...
    # Check that memory is shared
    assert np.shares_memory(array_np, np.from_dlpack(array.getLocalView())), "Memory should be shared between two NumPy arrays"
